...
export function PaymentsTable() {
    ...
    const ticketsQuery = useMemoFirebase(() => {
        ...
        return query(collection(firestore, "tickets"), where("status", "in", ["COMPLETED", "REFUND_PROCESSED"]));
    }, [firestore]);
    ...
    const paymentsData = React.useMemo(() => {
        ...
        let derivedPayments = tickets.map(ticket => {
            const amount = ticket.price || 0;
            const commission = amount * (ticket.commissionPercent || 10) / 100;
            const netAmount = amount - commission;
            let status = "HELD";
            if (ticket.status === 'COMPLETED') status = 'READY_FOR_PAYOUT';
    ...
    <div className="space-y-4">
      ...
      <div className="border rounded-lg overflow-hidden">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
                <TableRow><TableCell colSpan={8} className="text-center"><Skeleton className="h-24 w-full" /></TableCell></TableRow>
                ...
                <TableRow><TableCell colSpan={8} className="text-center">No payment data available.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
